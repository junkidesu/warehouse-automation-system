...
import Data.Time
import Database.Operations (userByUsername)
...
authServer conns jwts = login
 where
  ...
  login credentials = do
    foundUser <- liftIO $ userByUsername conns (T.strip . C.username $ credentials)
        ...
        let passwordCheck =
         ...
         in case passwordCheck of
              ...
              PasswordCheckSuccess -> do
                let
                  authUser = AU.AuthUser (U.id user) (U.username user)
                ...
                case eitherToken of
                  ...
                  Right tokenBS -> do
                    let token = T.pack . BS.toString $ tokenBS
                        authResponse = AR.AuthResponse token
                    return authResponse
